...
public class NotificationUtil {
    /**
     * 清除通知（Notification.FLAG_NO_CLEAR;不会被清理）
     *
     * @param activity
     */
    public static void clearNotifications(Activity activity) {
        NotificationManager notiManager = (NotificationManager) activity.getSystemService(Context.NOTIFICATION_SERVICE);
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
            StatusBarNotification[] statusBarNotifications = notiManager.getActiveNotifications();
            for (StatusBarNotification statusBarNotification : statusBarNotifications) {
                if ((statusBarNotification.getNotification().flags & Notification.FLAG_NO_CLEAR) == Notification.FLAG_NO_CLEAR) {
                    continue;
                }
                notiManager.cancel(statusBarNotification.getTag(), statusBarNotification.getId());
            }
        } else {
            notiManager.cancelAll();
        }
       // BadgeUtil.resetBadgeCount(activity.getApplicationContext());
    }
